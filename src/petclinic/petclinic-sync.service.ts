...
export class PetclinicSyncService {
  ...
  async sync(installationId: string, input: Record<string, any>, actorId: string, actorType = 'PLATFORM_ADMIN') {
    ...
    if (!connection.active) throw new ConflictException('PETCLINIC_CONNECTION_INACTIVE');
  ...
  async verify(installationId: string, externalReferenceId: string, scheduledAt: Date): Promise<boolean> {
    const connection = await this.prisma.petclinicConnection.findUnique({ where: { installationId } });
    if (!connection?.active || !externalReferenceId.startsWith('appointment:')) return false;
    const id = externalReferenceId.slice('appointment:'.length);
    const from = new Date(scheduledAt.getTime() - 2 * 86400_000); const to = new Date(scheduledAt.getTime() + 9 * 86400_000);
    const appointment = (await this.client.list(connection, from, to)).find((row) => row.id === id);
    return appointment ? this.decide(connection.allowedBranchIds, connection.pilotAllowedPhoneHashes, appointment).eligible : false;
  }
  ...
  private decide(branches: string[], phoneHashes: string[], appointment: PetclinicAppointment) {
    ...
    else { try { if (!phoneHashes.includes(this.crypto.phoneHash(normalizeVietnamPhone(appointment.phone)))) reason = 'NOT_IN_PILOT_ALLOWLIST'; } catch { reason = 'PHONE_INVALID'; } }
